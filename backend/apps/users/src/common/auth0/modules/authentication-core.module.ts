...
export class AuthenticationCoreModule {
  ...
  static forRootAsync(options: AuthenticationAsyncOptions): DynamicModule {
    const provider: Provider<any> = {
      inject: [AUTH_MODULE],
      provide: AUTH_CLIENT,
      useFactory: (authOptions: AuthenticationClientOptions) =>
        getAuthenticationClient(authOptions)
    };
  ...
  private static createAsyncOptionsProvider(
    options: AuthenticationAsyncOptions
  ): Provider {
    ...
    return {
      inject: options.useExisting
        ...
        : [],
      provide: AUTH_MODULE,
      useFactory: (optionsFactory: AuthenticationOptionsFactory) =>
